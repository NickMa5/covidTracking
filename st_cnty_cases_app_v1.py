...
# State graphs
    ...
    html.H2("Total positive cases in state.", style={"text_align":"left"}),
    dcc.Graph('st_line_chart', figure={}), # new line chart
    html.Br(),

    html.H2("Daily positive cases in state.", style={"text_align":"left"}),
...
# County graphs
    ...
    html.H2("Total positive cases in county.", style={"text_align": "left"}),
    dcc.Graph('cnty_line_chart', figure={}),  # new line chart
    html.Br(),

    html.H2("Daily positive cases in county.", style={"text_align": "left"}),
    dcc.Graph('cnty_bar_chart', figure={})
])

# ------------------------------------------------------------------------------
# Connect the Plotly state graphs with Dash Components
@app.callback(
    [Output(component_id='st_container', component_property='children'),
     Output(component_id='st_bar_chart', component_property='figure'),
     Output(component_id='st_line_chart', component_property='figure')], # new for line chart
...
def update_state_graph(state_slctd):
    ...
    fig2 = px.line(
        ...
        x='date',
        y='cases',
        template='plotly_dark') # new for line graph

    return container, fig, fig2

# Connect the Plotly county graphs with Dash Components
@app.callback(
    [Output(component_id='cnty_container', component_property='children'),
     ...
     Output(component_id='cnty_line_chart', component_property='figure')],
